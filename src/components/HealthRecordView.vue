<template>
  <view class="health-record">
    ...
    <view v-else-if="profile" class="record-content">
      <!-- Tab 切换 -->
      <view class="tab-bar">
        <view
          v-for="tab in tabs"
      ...
      <view v-if="activeTab === 'visits'" class="tab-content">
        ...
        <view v-else class="timeline">
          <view v-for="(visit, index) in profile.medicalVisits" :key="visit.id" class="timeline-item">
      ...
      <view v-if="activeTab === 'exams'" class="tab-content">
        ...
        <view v-else class="exam-list">
          <view v-for="exam in profile.examRecords" :key="exam.id" class="exam-card">
            <view class="exam-top">
            ...
            <view class="exam-toggle" @tap="toggleExam(exam.id)">
              ...
              <ChevronDown :size="14" color="#0D9488" :class="{ 'toggle-icon-up': expandedExams[exam.id] }" class="toggle-icon" />
            </view>
            <view v-if="expandedExams[exam.id]" class="indicator-list">
              <view v-for="ind in exam.indicators" :key="ind.name" class="indicator-row">
      ...
      <view v-if="activeTab === 'risks'" class="tab-content">
        ...
        <view v-else class="risk-list">
          <view v-for="risk in profile.risks" :key="risk.id" class="risk-card">
            ...
            <view v-if="risk.relatedVisits && risk.relatedVisits.length > 0" class="risk-related">
              ...
              <view v-for="visitId in risk.relatedVisits" :key="visitId" class="risk-related-item" @tap="goToVisit(visitId)">
    ...
    <!-- 无数据状态 -->
    <view v-else class="empty-state" style="padding-top: 80px;">
...
import {
  Building2, User, FileText, FileX, ChevronDown, ShieldCheck, Lightbulb,
  Activity, ClipboardList, ShieldAlert,
...
.tab-active { background: rgba(13, 148, 136, 0.08); }
...
.dept-tag { padding: 2px 8px; border-radius: 6px; }
...
.dept-tag-text { font-size: 11px; font-weight: 500; font-family: "Noto Sans SC", sans-serif; }
...
.visit-hospital-row, .visit-doctor-row { display: flex; align-items: center; gap: 5px; }
.visit-hospital, .visit-doctor { font-size: 12px; color: #9CA3AF; font-family: "Noto Sans SC", sans-serif; }

.visit-diag-row { display: flex; align-items: flex-start; gap: 6px; }
...
.visit-diag { font-size: 13px; font-weight: 600; color: #1A1A1A; font-family: "Noto Sans SC", sans-serif; line-height: 1.4; }
...
.exam-top { display: flex; align-items: flex-start; justify-content: space-between; }
.exam-meta { display: flex; flex-direction: column; gap: 6px; }
.exam-date { font-size: 16px; font-weight: 700; color: #1A1A1A; font-family: "DM Sans", sans-serif; }
...
.exam-type-text { font-size: 11px; font-weight: 500; color: #0D9488; font-family: "Noto Sans SC", sans-serif; }
...
.exam-abnormal-num { font-size: 20px; font-weight: 800; color: #EF4444; font-family: "DM Sans", sans-serif; line-height: 1; }
.exam-abnormal-label { font-size: 11px; color: #EF4444; font-family: "Noto Sans SC", sans-serif; }
.exam-institution-row { display: flex; align-items: center; gap: 5px; }
.exam-institution { font-size: 12px; color: #9CA3AF; font-family: "Noto Sans SC", sans-serif; }
.exam-summary { font-size: 13px; color: #6B7280; line-height: 1.6; font-family: "Noto Sans SC", sans-serif; }
...
.exam-toggle-text { font-size: 12px; font-weight: 500; color: #0D9488; font-family: "Noto Sans SC", sans-serif; }
.toggle-icon { transition: transform 0.25s ease; }
.toggle-icon-up { transform: rotate(180deg); }
...
.ind-name { flex: 1; font-size: 13px; color: #1A1A1A; font-family: "Noto Sans SC", sans-serif; }
.ind-value { font-size: 13px; font-weight: 600; color: #1A1A1A; font-family: "DM Sans", sans-serif; margin-right: 10px; }
.ind-status { padding: 1px 7px; border-radius: 5px; }
.is-normal { background: #D1FAE5; }
.is-high { background: #FEF3C7; }
.is-low { background: #DBEAFE; }
.ind-label { font-size: 10px; font-weight: 500; font-family: "Noto Sans SC", sans-serif; }
...
.risk-name-area { display: flex; align-items: center; gap: 8px; }

.risk-level-dot { width: 8px; height: 8px; border-radius: 4px; flex-shrink: 0; }
.rld-high { background: #EF4444; }
.rld-medium { background: #F59E0B; }
.rld-low { background: #10B981; }

.risk-name { font-size: 15px; font-weight: 700; color: #1A1A1A; font-family: "Noto Sans SC", sans-serif; }
.risk-level-tag { padding: 2px 10px; border-radius: 6px; }
.rlt-high { background: rgba(239, 68, 68, 0.1); }
.rlt-medium { background: rgba(245, 158, 11, 0.1); }
.rlt-low { background: rgba(16, 185, 129, 0.1); }
.risk-level-text { font-size: 11px; font-weight: 600; font-family: "Noto Sans SC", sans-serif; }
.rlt-high .risk-level-text { color: #EF4444; }
.rlt-medium .risk-level-text { color: #F59E0B; }
.rlt-low .risk-level-text { color: #10B981; }

.risk-desc { font-size: 13px; color: #6B7280; line-height: 1.6; font-family: "Noto Sans SC", sans-serif; }
...
.suggest-icon { flex-shrink: 0; margin-top: 1px; }
.risk-suggest { font-size: 13px; color: #0D9488; line-height: 1.6; font-family: "Noto Sans SC", sans-serif; }
...
.risk-related-label { font-size: 11px; font-weight: 600; color: #9CA3AF; font-family: "Noto Sans SC", sans-serif; }
.risk-related-item { display: flex; align-items: center; gap: 5px; &:active { opacity: 0.7; } }
